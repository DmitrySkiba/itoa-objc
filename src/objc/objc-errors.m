...
static void _objc_crashlog(const char *message)
{
    ...
    if (!__crashreporter_info__) {
        asprintf(&newmsg, "objc[%d]: %s", getpid(), message);
    } else {
        asprintf(&newmsg, "%s\nobjc[%d]: %s", __crashreporter_info__, getpid(), message);
    }
 ...
 */
__private_extern__ void _objc_error(id self, const char *fmt, va_list ap) 
{ 
    char *buf1;
    char *buf2;

    vasprintf(&buf1, fmt, ap);
    asprintf(&buf2, "%s: %s\n", object_getClassName(self), buf1);
    _objc_syslog(buf2);
    _objc_crashlog(buf2);
