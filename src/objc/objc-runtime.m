...
__private_extern__ pthread_t DebuggerModeThread = 0;
...
int gdb_objc_startDebuggerMode(uint32_t flags)
{
    ...
    if (! DebuggerMode) {
        ...
        if (mode == DEBUGGER_OFF) {
            ...
            DebuggerMode = mode;
            DebuggerModeCount = 1;
            DebuggerModeThread = pthread_self();
            return 1;
        }
    } 
        ...
        if (pthread_self() == DebuggerModeThread) {
            DebuggerModeCount++;
            return 1;
        } else {
...
{
    if (DebuggerMode  &&  pthread_self() == DebuggerModeThread) {
